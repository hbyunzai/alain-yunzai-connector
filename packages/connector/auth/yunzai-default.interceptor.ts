...
function handleData(injector: Injector, ev: HttpResponseBase, req: HttpRequest<any>, next: HttpHandlerFn): Observable<any> {
  ...
  switch (ev.status) {
    ...
    case 401:
      ...
      return tryRefreshToken(injector, ev, req, next).pipe(catchError(() => toLogin(injector) as NzSafeAny));
    case 403:
    case 404:
    case 500:
...
export const yunzaiDefaultInterceptor: HttpInterceptorFn = (req, next) => {
  const config = mergeYunzaiConfig(inject(YunzaiConfigService));
  let url = req.url;
  if (!req.context.get(IGNORE_BASE_URL) && !url.startsWith('https://') && !url.startsWith('http://')) {
    url = config.connector?.backstage + (config.connector?.backstage!.endsWith('/') && url.startsWith('/') ? url.substring(1) : url);
  }
